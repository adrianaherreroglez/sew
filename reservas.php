...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['presupuestar'])) {
    ...
    if ($fechaInicioPresupuesto < $fechaActual) {
        ...
        // Buscar precio unitario directamente por id
        $precioUnitario = 0;
        foreach ($recursos as $r) {
            if ($r['id'] === $recursoIdPresupuesto) {
                $precioUnitario = floatval($r['precio']);
                $recursoPresupuesto = $r; // Guarda el recurso para mostrar después
                break;
            }
        }
        ...
        // Guardar datos en sesión para confirmar reserva luego
...
<section>
  <h2>Presupuestado</h2>
  <h3>Presupuesto para: <?= htmlspecialchars($datosReserva['nombre_recurso']) ?></h3>
  <p>Precio estimado: <?= number_format($datosReserva['precio'], 2) ?> €</p>
  <p>Fecha de inicio: <?= date('d/m/Y H:i', strtotime($datosReserva['fecha_inicio'])) ?></p>
  <p>Fecha de fin: <?= date('d/m/Y H:i', strtotime($datosReserva['fecha_fin'])) ?></p>
...
<form method="POST" action="php/controllers/ReservaController.php">
  ...
  <select name="reserva_id">
    <?php foreach ($reservas as $reserva): ?>
      <option value="<?= htmlspecialchars($reserva['id']) ?>">
   ...
   <nav>
          <button name="anular">Anular</button>
    </nav>
...
<section>
<form method="POST">
  <label>Selecciona un recurso turístico:</label>
  <select name="recurso_id" required>
    <?php foreach ($recursos as $r): ?>
      <option value="<?= htmlspecialchars($r['id']) ?>" data-precio="<?= htmlspecialchars($r['precio']) ?>">
